...
ACTUAL_ROWS_IN_INPUT_FILE = 5000  # Number of lines in the input file
...
if __name__ == '__main__':
    try:
        ...
        with testHelper.getOpenConnection(dbname=DATABASE_NAME) as conn:
            ...
            choice = input("Press enter to Delete all Range Partition tables? ")
            if choice == '':
                testHelper.deleteAllPublicTables(conn)
            ...
            choice = input("Press enter to Delete all Round Robin Partition tables? ")
            if choice == '':
                testHelper.deleteAllPublicTables(conn)
